...
tool_bin="/usr/bin/uzbl-browser"
basedir=`pwd`
configdir="$basedir/uzbl-config"
export UZBLTEST_CONFIG_DIR=$configdir
config_file="$configdir/config"
data_storage="$basedir/uzbl-run/"`date +%Y%m%d_%H-%M-%S`
...
f_request_server_info(){ #{{{1
  ...
  _info=$(wget -T 0 -t 0 -qO- "$server_stat_url?all")
  echo -e -n "$_info\n"
}
...
f_run_test(){ #{{{1
  ...
  export UZBLTEST_URLHASH=$urlhash
  ...
  do
    ...
    ($tool_bin -c $config_file $url >> /dev/null 2>&1)& >> /dev/null 2>&1
    sleep $intersleep;
  done
  ####### END OF TEST RUNS ############
  # request server info
  ...
  f_gp_test_base $gp_test_output "uzbl test $url" > $gp_test_input;
